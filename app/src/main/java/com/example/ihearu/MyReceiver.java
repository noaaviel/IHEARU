    ...
    public void onReceive(Context context, Intent intent) {
        switch (intent.getAction()) {
            case ACTION_SMS_SENT:
                ...
                Log.d(com.example.ihearu.MainScreenActivity.TAG, "Message Sent");

                break;
            case ACTION_SMS_DELIVERED:
                Toast.makeText(context, "Message Delivered", Toast.LENGTH_LONG).show();

                Log.d(com.example.ihearu.MainScreenActivity.TAG, "Message Delivered");
                break;
            default:
    ...
    private void powerButtonPressCount(Context context) {
        ...
        if (mCount == 0 && mTimeStamp == 0) {
            ...
            if ( (ts - mTimeStamp) <= 3 ) {
                ...
                Log.d(com.example.ihearu.MainScreenActivity.TAG, "Going towards...");
